...
interface Exercise {
  ...
  /** AIが「実行」するときの立場 */
  actAs: string;
  /** 前提資料（元の文章・場面など）。アプリ側の given と揃えること。
      これが無いとユーザーは書きようがないし、実行役も材料が無い */
  given?: string;
}
...
const EXERCISES: Record<string, Exercise> = {
  "prompt-basics": {
    ...
    given:
      "【元の文章】弊社は今期よりDX推進の一環として、オンプレミス環境の基幹システムをSaaS型CRMへリプレイスします。各部署はマイグレーションに先立ち、マスタデータのクレンジングを完了してください。\n【読み手】営業部の新入社員。ITの専門用語はほとんど通じない",
    criteria: [
  ...
  "risk-redact": {
    ...
    actAs: "その相談に実際に答える相手",
    given:
      "【伏せたい相談】取引先の山田製作所（従業員300名の製造業）との契約、月額120万円を来期から140万円に値上げしたい。購買部長の佐藤さんにどう切り出すか相談したい。\n（採点時：山田製作所・佐藤・120万/140万が伏せられていれば満点扱い。伏せ方は記号でもダミー名でもよい）",
    criteria: [
  ...
  "rag-brief": {
    ...
    given:
      "【場面】社内の「在宅勤務規程」のPDFをAIに渡して、「在宅勤務は週何日までできるか」を答えさせたい。規程に書いていないことを、それらしく作られては困る",
    criteria: [
  ...
  "prompt-role": {
    brief: "役割を与えて、下の企画の弱点を洗い出させる指示を書く（生成させるのではなく批評させる）",
    actAs: "指示された役割になりきって批評する人",
    given:
      "【企画】社内の若手向けに、月1回「先輩と晩ごはん」の制度を作りたい。目的は若手の定着率アップ。予算は1人3,000円まで、参加は自由。",
    criteria: [
      "AIに与える役割が具体的か",
...
export async function POST(req: Request) {
  ...
  const user = `# お題
${ex.brief}
${ex.given ? `\n# 前提資料（ユーザーにも見せているもの。手順1で成果物を書くときの材料にする）\n${ex.given}\n` : ""}
# 採点の観点
