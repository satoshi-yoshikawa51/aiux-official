...
朝焼け・夕暮れ・雨・セピア・深夜・雪は、絵が無かった頃の名残で
**素の教室に色を重ねているだけ**（`art` を持たない）。**Nが10枚そろった
いま、役割がかぶっている**（Nだけで15枚になり、N10/R7/SR3 の計画から外れる）。

引退させるなら、`ame`（雨）と `yuki`（雪）が持っている降りものの演出は
R側で引き取れる（`neon-rain` に雨、`cabin-fire` に火の粉）。ただし
**雪の舞台はいまの20枚の中に無い**ので、雪だけは残すか、Rを1枚
差し替えるかを決めること。
