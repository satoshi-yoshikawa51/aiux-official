...
|---|---|---|---|---|
| `courtyard.jpg` | 中庭のベンチ | N | 下を16%削る（0.897） | 0.53 |
| `sakura.jpg` | 桜並木 | R | 削らない（0.753） | 0.60 |

どちらも `--ar 3:4` の 1856x2464。桜は道の消失点が絵の中に入っているので、
削らずそのまま使う。中庭は旧い指示（引きの構図）で描いたぶん床が多く、
下を削って比率を稼いでいる。
