...
N10 / R7 / SR3 のうち7枚。**消失点を強く指示してからは、7枚とも
`horizon: 0.5` のまま噛み合っている**（調整不要だった）。

| ファイル | テーマ | レア度 | 切り方 | horizon |
|---|---|---|---|---|
| `classroom.jpg` | いつもの教室（初期所持） | N | 削らない（0.753） | 0.50 |
| `library.jpg` | 図書室の窓辺 | N | 削らない（0.753） | 0.50 |
| `corridor.jpg` | 放課後の廊下 | N | 削らない（0.753） | 0.50 |
| `computer-room.jpg` | コンピュータ室 | N | 削らない（0.753） | 0.50 |
| `home-desk.jpg` | 家のデスク | N | 削らない（0.753） | 0.50 |
| `courtyard.jpg` | 中庭のベンチ | N | 下を16%削る（0.897） | 0.48 |
| `sakura.jpg` | 桜並木 | R | 削らない（0.753） | 0.50 |

中庭だけは旧い指示（引きの構図）で描いたぶん床が多く、下を削って比率を稼いでいる。
...
## 色を重ねるだけのテーマは、いずれ引退させる

朝焼け・夕暮れ・雨・セピア・深夜は、絵が無かった頃の名残で
**素の教室に色を重ねているだけ**（`art` を持たない）。残りのN——昇降口・
カフェ・屋上（曇り）・会議室——が描けたらNが10枚そろうので、そこで
役割がかぶる。引退させるか、絵つきに描き直すかを決めること。
