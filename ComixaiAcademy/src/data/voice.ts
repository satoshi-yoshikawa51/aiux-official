/* ============================================================
   ...
   **6人ぶん、全部そろっている。** このファイルの場面ごとのセリフ
   （クイズ5・修了試験6・復習5・エンディング1・小話6・ホーム4）も、
   レッスン本文の sayByAvatar（courses/ 79枚）も、称号（badges.ts 7段）も。

   **例外はレッスン本文の3枚だけ**（work.ts の一歩目・二歩目・三歩目の
   1枚目）。そこは職種別（sayByRole）が入っていて、相棒別を書くと
   職種別を上書きしてしまう（→ courses/index.ts の resolveCard）。
   「あなたの仕事は〜」と相手の仕事に触れる回なので、職種別に譲ってある。
...
/* ———————————————— ホームのひとこと ————————————————
   ホームのフキダシに出る、ふだんの一言（app/(tabs)/index.tsx）。
   つついたときの小話とは別で、**何もしていないときに出ている言葉**。

   ▍ここも相棒ごとに書く
   長いあいだ、この4本だけ先輩の口調で直書きされていた。ホームは
   いちばん長く見る画面なので、**選んだ相棒と違う人がしゃべっている**
   のがそのまま残っていた（案内・称号・レッスンは書き分け済み）。

   ▍差し込む言葉があるので関数にする
   職種名・日数・レッスン名が入る。テンプレの穴埋めにすると、埋め
   忘れても型で気づけない。Line を返す関数にして、say() に通す。 */

export const HOME_VOICE = {
  /** 全部終わった人。**「終わり」と言い切らない**（実機で「終わりと
      言われるとアプリを消す」と出ている） */
  allDone: {
    common: 'ぜんぶ終わったね。……よくやった。次の章はいま用意してる。それまでは現場で使って。',
    byAvatar: {
      ottori:
        'ぜんぶ、終わりましたね。……よく続けました。次の章はいま用意しています。それまでは、現場で使ってみてください。',
      nekketsu:
        '全部やりきったな。……よくやった。次の章はいま作ってる。それまでは現場で使え。',
      otenba: 'ぜんぶ終わったじゃん。……やるね。次の章はいま作ってるとこ。それまでは現場で使お。',
      kanroku:
        'ぜんぶ終わったか。……よくやったな。次の章はいま用意している。それまでは現場で使うといい。',
      neko: 'ぜんぶ終わったね！ すごい！ 次の章はいま作ってるんだって。それまでは、しごとで使ってみて！',
    },
  } as Line,

  /** まだ1本もやっていない人。職種を決めた直後に出る */
  start: (role: string): Line => ({
    common: `${role}ね。なら、話が早い。まず1本やってみて。`,
    byAvatar: {
      ottori: `${role}なんですね。それなら、話が早いです。まず1本、やってみましょう。`,
      nekketsu: `${role}か。なら話は早い。まず1本、やってみようぜ。`,
      otenba: `${role}なんだ。じゃ話早いね。とりま1本やってみよ。`,
      kanroku: `${role}か。なら、話が早いな。まず1本やってみよう。`,
      neko: `${role}なんだ！ じゃあ話がはやいね。まず1本、やってみよ！`,
    },
  }),

  /** 3日以上続いている人 */
  streak: (days: number): Line => ({
    common: `${days}日続いてるね。……その調子よ。`,
    byAvatar: {
      ottori: `${days}日、続いていますね。……その調子です。`,
      nekketsu: `${days}日続いてるな。……その調子だ！`,
      otenba: `${days}日つづいてるじゃん。……いい感じ。`,
      kanroku: `${days}日続いているな。……その調子だ。`,
      neko: `${days}日つづいてる！ すごいじゃん、その調子！`,
    },
  }),

  /** ふだんの一言。次にやるレッスンを言う */
  next: (title: string): Line => ({
    common: `次は「${title}」よ。`,
    byAvatar: {
      ottori: `次は「${title}」ですね。`,
      nekketsu: `次は「${title}」だ。いくぞ。`,
      otenba: `次は「${title}」だね。`,
      kanroku: `次は「${title}」だな。`,
      neko: `次は「${title}」だって！`,
    },
