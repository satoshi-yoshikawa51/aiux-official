...
/* ▍救済（天井）
   最後の1本（1.11%）を素の確率だけで狙うと、期待値でも90回・
   運が悪いと数百回まわしても出ない。課金の無いガチャなので、
   渋らせて得るものが無い。**この回数つづけて新しいものが出なかったら、
   次の1回は持っていない景品から確定で出す**。これで、どの1本も
   狙い始めから最大 PITY_AFTER+1 回で必ず出る。
   レア度は見ない——終盤は残りがSRだけになるので、レア度を守ると
   救済がいちばん要るところで効かない。抽選そのものは
   store/progress.tsx の spinGacha がやる（持ち物を見る必要があるため）。
   連続ハズレの数は state.dryStreak に持つ。 */
