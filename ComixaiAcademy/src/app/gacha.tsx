  ...
  return (
    ...
    <View style={{ flex: 1 }}>
      ...
      {/* 1本目を終えた人への案内。まわす前と後で言うことが変わる。
          ...
          ▍バンドの背が変わったら、マシンを測り直す
          マシンの onLayout は「親の中での自分の位置」が変わった時しか
          発火しない。バンドはマシンの**上**にいるので、文言の入れ替わりや
          「わかった」で背が変わると、マシンは親ごとずれて onLayout が
          鳴らないまま窓座標が古くなる——手前に重ねる取り出し口の板が
          置き去りになり、**筐体が二重に見える**（実機の案内中の1回目で発生）。
          バンドを包んで、寸法が動いたらこちらから測り直す */}
      <View
        onLayout={() => {
          measureMachine();
          measureLayer();
      ...
      {open ? (
        <View
          ...
          <Text style={F.tiny}>
            ...
            {state.dryStreak > 0 ? `（いま${state.dryStreak}回つづけてダブり）` : ''}。
          </Text>
        </View>
      ) : null}
    </View>
