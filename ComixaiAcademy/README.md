...
初回起動のときだけ `/opening` が出て、サイトの `/history` の年表を1コマずつ送れる。

**この画面は待ち時間をごまかすためのものではなく、実際に先読みしている。**
ストアからのダウンロードはOSの仕事なのでアプリからは見せられない。待ちが出るのは
初回のアセット読み込み（アバターのGLB 1.9MB＋テクスチャ＋舞台の背景）で、放っておくと
ホームに着いてから読み込みが始まり、キャラが出るまで数秒かかる。オープニングが
`Asset.loadAsync` で先に落とすので、飛ばした人もホームで待たされない。
**「スキップ」は読み込みが終わるまで出ない**（出しても得がないため）。
...
- `src/data/emaki.ts` … 年・タイトル・本文・ツッコミ・トーン（生成物）
- `assets/images/emaki/*.webp` … コマ絵（サイトの public/history からコピー、14枚で約1.2MB）

サイト側は mp4 も持っているが、8MBあるので**取り込んでいない**。1コマずつボタンで送る
見せ方なら静止画で足りる。

2回目以降は出ない（`seenOpening` を保存している）。**記録をリセットしても出し直さない**——
初回だけの導入であって、見返したいものではないため。
