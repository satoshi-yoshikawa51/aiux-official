...
ホームのアバターは「黒帯 → 余白 → コマ → 次のカセット → タブバー」に挟まれて
位置が決まりますが、一幕には**そのどれも無い**ので、数字で合わせるしかありません。
`intro.tsx` の `HOME_RATIO` / `HOME_SIDE` / `HOME_FOOT` / `HOME_HEAD` がそれで、
ホーム（案内を閉じた状態）の実測から出しています。

| 画面 | ホームの枠 | 一幕の枠 | 足元から下まで |
|---|---|---|---|
| 390x844 | 330x364 | 330x363 | 230 |
| 375x667 | 287x316 | 290x319 | 208 |
| 430x932 | 370x408 | 370x407 | 230 |

**ホーム側の余白・カセット・`AVATAR_RATIO` を触ったら、ここも測り直すこと。**
確かめ方は、一幕とホームで `document.querySelector('canvas').getBoundingClientRect()`
を突き合わせるのがいちばん早いです。
