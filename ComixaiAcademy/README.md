...
- **タブアイコン** — `src/components/icons.tsx` のオリジナル（黒地に白抜き、選択中は赤の丸ベタ）。
...
歯車や星は目分量だと歪むので、座標を計算して作る。確認用のスクリプトがある:

```bash
node tools/build-icons.mjs   # 形を作って sheet.png（22/44/88px の一覧）を書き出す
```

**22pxで実際にラスタライズしてから拡大して見ること。** 大きい絵で判断すると、
端末では潰れている、ということが起きる。仕上がった `d`（パス）を
`src/components/icons.tsx` の `PATHS` に貼れば差し替え完了。
見た目の大きさの差は同ファイルの `OPTICAL` で微調整する。
