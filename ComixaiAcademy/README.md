...
### 最上位の称号は「やり込みだけで届く」ところに置く

もとはバッジが14個で、最上位の `AIマスター` が14＝**全部**でした。つまり
`streak-7`（7日連続）が必須で、**1日で17本やり切ったヘビーユーザーでも7日目まで
マスターになれない**。頑張った人ほど「あと2個」の中身が待ち時間だと気づいて萎える
作りでした。

いまはバッジが20個あり、**1日で取り切れるのは16個**（連続日数2つと復習2つは、
日をまたがないと取れない）。最上位をちょうど16に置いてあるので、やり込めば
その日のうちに届きます。残る4つは称号の先にあるコレクション。

| 称号 | 必要バッジ |
|---|---|
| AI見習い / 研修生 / 相棒 | 0 / 3 / 6 |
| AI使い / 職人 / 師範 | 9 / 11 / 13 |
| AIマスター | 16（＝1日で取り切れる上限） |
...
トークン収めには★が付かないので、17本を分母にすると**永久に達成できない条件**に
なります（`STARRED_LESSON_IDS` が16本を返す）。
